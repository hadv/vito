...
import { HelpGuide, SafeInfoDisplay } from './components';
...
class VimApp {
    ...
    fields.forEach(field => {
      ...
      input.addEventListener('input', () => {
        if (field.id === 'tx-to') {
          ...
          this.txFormData!.data = (input as unknown as HTMLTextAreaElement).value;
        }
      });
  ...
  private displaySafeInfo(safeInfo: SafeInfo): void {
    // Create a new SafeInfoDisplay instance and use it to display the safe info
    const safeInfoDisplay = new SafeInfoDisplay(
      this.buffer,
      this.provider,
      this.safeAddress || undefined, // Convert null to undefined
      this.signerAddress || undefined, // Convert null to undefined
      this.selectedNetwork,
      this.cachedSafeInfo || undefined, // Convert null to undefined
      this.commandInput,
      this.showTokenSendingScreen.bind(this)
    );
    
    safeInfoDisplay.display(safeInfo);
  }
  ...
  private showTokenSendingScreen(token: Token): void {
    ...
    formContainer.appendChild(form);
    form.appendChild(helperText);
    // tokenInfoText no longer exists
